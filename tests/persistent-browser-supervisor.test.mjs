...
test('persistent supervisor accepts Google as an explicit engine before browser startup', async (t) => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'lite-job-google-engine-'));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const input = await createInput(directory);
