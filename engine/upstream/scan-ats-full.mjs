...
const DATASET_API_BASE = 'https://api.github.com/repos/Feashliaa/job-board-aggregator/contents/data';
...
const VALUE_FLAGS = ['--since', '--limit', '--ats', '--seeds', '--md-out', '--concurrency', '--shard-index', '--shard-count'];
...
const USAGE = `Usage:
  ...
  node scan-ats-full.mjs --ats greenhouse,workday  # subset of sources
  node scan-ats-full.mjs --limit 200          # max companies per ATS (default: all)
  node scan-ats-full.mjs --concurrency 4      # lower request concurrency for cautious scans
  node scan-ats-full.mjs --shard-index 0 --shard-count 4  # deterministic quarter of each ATS directory
  node scan-ats-full.mjs --dry-run            # preview without writing files
  node scan-ats-full.mjs --ignore-history     # include URLs seen by other students/runs
  node scan-ats-full.mjs --liveness           # Playwright-verify matches before writing
  node scan-ats-full.mjs --verbose            # log per-board fetch failures
  node scan-ats-full.mjs --md-out <dir>       # also write a dated markdown digest to <dir>
  node scan-ats-full.mjs --help               # print this usage block and exit`;
...
function parseArgs(argv) {
  ...
  const sinceDays = Math.max(1, Math.min(180, Number(valueOf('--since')) || 3));
  const limit = Number(valueOf('--limit')) || Infinity;
  const concurrency = Math.max(1, Math.min(20, Number(valueOf('--concurrency')) || DEFAULT_CONCURRENCY));
  const shardCount = Math.max(1, Math.min(100, Number(valueOf('--shard-count')) || 1));
  const shardIndex = Math.max(0, Math.min(shardCount - 1, Number(valueOf('--shard-index')) || 0));
...
export function parseGitHubContentsPayload(payload) {
  if (!payload || payload.encoding !== 'base64' || typeof payload.content !== 'string') return null;
  try {
    const parsed = JSON.parse(Buffer.from(payload.content.replace(/\s/g, ''), 'base64').toString('utf8'));
    return Array.isArray(parsed) ? parsed : null;
  } catch { return null; }
}

async function fetchCompanyDataset(name, rawUrl) {
  const apiUrl = `${DATASET_API_BASE}/${name}_companies.json?ref=main`;
  try {
    const payload = await fetchJson(apiUrl, {
      timeoutMs: 20_000,
      headers: { accept: 'application/vnd.github+json', 'x-github-api-version': '2022-11-28' },
    });
    const parsed = parseGitHubContentsPayload(payload);
    if (parsed) return parsed;
  } catch (error) {
    console.error(`⚠️  ${name}: GitHub Contents API failed — ${error.message}; trying raw mirror`);
  }
  return await fetchJson(rawUrl, { timeoutMs: 30_000 });
}
...
async function loadCompanyList(name, url) {
  ...
  try {
    const data = await fetchCompanyDataset(name, url);
...
// Stable directory partitioning makes repeated capped scans cover new boards
// instead of repeatedly sampling the same companies. Sharding happens before
// the per-shard limit, so shard 0/4 and shard 1/4 never overlap.
export function selectCompanyShard(list, { shardIndex = 0, shardCount = 1, limit = Infinity, shuffle = false } = {}) {
  const count = Math.max(1, Math.floor(shardCount));
  const index = Math.max(0, Math.min(count - 1, Math.floor(shardIndex)));
  const shard = list.filter((_, itemIndex) => itemIndex % count === index);
  return sampleCompanies(shard, limit, shuffle);
}
...
export async function runSeedScan(seedId, opts, ctx, seenUrls, label) {
  ...
  await parallelEach(capped, opts.concurrency, async (company) => {
...
async function main() {
  ...
  const seenUrls = opts.ignoreHistory ? new Set() : loadSeenUrls().seen;
  ...
  for (const name of opts.ats) {
    ...
    const shardSize = selectCompanyShard(list, { ...opts, limit: Infinity, shuffle: false }).length;
    if (opts.limit < shardSize) capHit = true;
    const selectedCompanies = selectCompanyShard(list, opts);
    const entries = selectedCompanies.map(source.toEntry).filter(Boolean);
    totalCompaniesScanned += entries.length;
    log(`\n⚙  ${name} — ${entries.length} companies${status !== 'ok' ? ` (dataset: ${status})` : ''}`);

    let done = 0;
    let errors = 0;
    await parallelEach(entries, opts.concurrency, async (entry) => {
      try {
