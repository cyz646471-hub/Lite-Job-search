...
export async function runCnMarketDiscoveryCycle({
  ...
  searchDelayMs = 4_000, headless = false, batchId = '',
} = {}) {
  const resolvedOutput = path.resolve(outputDir);
  const queueFile = path.join(resolvedOutput, 'company-leads.json');
  const discovery = await discoverCnMarketCompanies({
    databaseFile, outputFile: queueFile, profileDir, role, industry, targetCount, searchDelayMs, headless,
  });
  ...
  const worker = await runPersistentBrowserSupervisor({
    ...
    role, industry, searchDelayMs, headless, searchEngine: 'baidu', allowSearchFallback: true,
  });
