...
候选在进入公司 Worker 前，按标准化公司名、别名、官方域名同 SQLite `Company` 去重；同一轮搜索结果也会去重。职友集、前程无忧、智联、BOSS、猎聘、牛客、高校就业网、新闻、广告和培训类结果只会被记录为排除原因，不能成为企业线索。
...
- 默认只使用百度可见搜索页；不使用百度 API、Apify，也不会因安全验证自动切换引擎。
- 查询之间最短间隔为 4 秒。百度出现安全验证时立即写出 `BLOCKED` 结果并保留已发现线索，等待人工完成验证后再运行。
